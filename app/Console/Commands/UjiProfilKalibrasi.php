...
class UjiProfilKalibrasi extends Command
{
    ...
    private function ujiSatu(CalibrationProfile $profil, ?Equipment $alat, User $teknisi): array
    {
        ...
        // Thermocouple / Termometer Gelas / Thermohygrometer juga tidak lewat
        // `preview` per titik: payload-nya `measurements[i].standar` &
        // `.uut`, bukan `.pembacaan` yang datar. Diadu ke payload datar,
        // ketiganya memulangkan NOL titik — dan nol titik di sini terbaca
        // seperti mesin hitungnya rusak, padahal yang salah bentuk payload
        // yang disusun perintah ini.
        //
        // Yang diperiksa sama seperti Enclosure: sesi tersimpannya lengkap —
        // jumlah hasil hitung sama dengan jumlah set point yang benar-benar
        // ada datanya.
        if ($profil instanceof ProfilSuhuPasangan) {
            if ($alat === null) {
            ...
            return $terhitung > 0 && $terhitung === $terisi
                ? ["{$terhitung}/{$terisi}", "U95 lengkap (sesi {$sesi->nomor_sesi})", true]
                : [
                    "{$terhitung}/{$terisi}",
        ...
        // Micrometer juga tidak lewat `preview` per titik: payload-nya
        // `measurements[i].mikro_balok` (tumpukan balok ukur) + `.mikro_pembacaan`,
        // bukan `.pembacaan` yang datar — dan SELURUH budget-nya lahir dari blok
        // tingkat-sesi di `spesifikasi_alat.micrometer` (pra-evaluasi, suhu,
        // kapasitas, resolusi). Diadu ke payload datar dia memulangkan NOL titik,
        // dan nol titik di sini terbaca seperti mesin hitungnya rusak padahal yang
        ...
        if ($profil instanceof MicrometerProfile) {
            ...
            return $terhitung > 0 && $terhitung === $terisi
                ? ["{$terhitung}/{$terisi}", "U95 sesi lengkap (sesi {$sesi->nomor_sesi})", true]
                : [
                    "{$terhitung}/{$terisi}",
                    sprintf('titik terisi %d tapi kehitung %d (sesi %s)', $terisi, $terhitung, $sesi->nomor_sesi),
                    false,
