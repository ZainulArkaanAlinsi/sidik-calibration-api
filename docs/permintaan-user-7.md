...
| **K23** | **Direktori luar: tetap `osm`, atau pindah ke `auto` dengan tagihan?** | Cakupan pencarian pelanggan baru. **Nol kode** — variabelnya sudah ada di `.env.example` dan `auto` sudah didukung `PilihanDriver`; yang tersisa satu baris `.env` + key Places. Tapi ini **membatalkan K16** (nol tagihan), jadi cuma pemilik proyek yang boleh memutuskannya. Rinciannya P1 `docs/pertanyaan-lab-data-pelanggan.md` |
